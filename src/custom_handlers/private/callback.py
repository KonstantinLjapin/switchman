...
async def calendar_callback_query(call: CallbackQuery, bot: AsyncTeleBot, logger: Logger) -> None:
    ...
    msg: Message = await bot.send_message(call.message.chat.id, text="```" + status + "```", parse_mode='MarkdownV2')
...
def register_custom_private_callback_query_handlers(bot: AsyncTeleBot):
