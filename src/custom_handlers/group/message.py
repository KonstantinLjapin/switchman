...
async def start_message(message: Message, bot: AsyncTeleBot, logger: Logger) -> None:
    ...
    msg: Message = await bot.send_message(chat_id=message.chat.id, text=text)
...
async def this_day_is(message: Message, bot: AsyncTeleBot, logger: Logger) -> None:
    ...
    await bot.delete_message(chat_id=message.chat.id, message_id=message.message_id)
    msg: Message = await bot.send_message(message.chat.id, text=status)
    await asyncio.sleep(60)
    await bot.delete_message(chat_id=message.chat.id, message_id=msg.message_id)
...
async def this_year_is(message: Message, bot: AsyncTeleBot, logger: Logger) -> None:
    await bot.send_message(chat_id=message.chat.id, text="Все дни для:", reply_markup=gen_markup())
    await bot.delete_message(chat_id=message.chat.id, message_id=message.message_id)
...
def register_chat_custom_message_handlers(bot: AsyncTeleBot):
    bot.register_message_handler(start_message, commands=["start"], pass_bot=True, is_group=True, state=None)
    bot.register_message_handler(this_day_is, commands=["this_day_is"], pass_bot=True, is_group=True, state=None)
    bot.register_message_handler(this_mount_is, commands=["this_mount_is"], pass_bot=True, is_group=True, state=None)
    bot.register_message_handler(this_year_is, commands=["this_year_is"], pass_bot=True, is_group=True, state=None)
    bot.register_message_handler(echo_message, func=lambda message: True, pass_bot=True, is_group=True, state=None)
